...
def start_training(char_name: str, char_class: str) -> str:
    ...
    print('команды: attack —атака, defence —защита или special —магия.')
...
def choice_char_class() -> str:
    ...
    while approve_choice != 'y':
        char_class = input('Выбери: Воитель—warrior, Маг—mage, Лекарь—healer:')
        if char_class == 'warrior':
            print('Воитель —воин ближнего боя. Сильный, выносливый и отважный')
        if char_class == 'mage':
            print('Маг — воин дальнего боя. Обладает высоким интеллектом')
        if char_class == 'healer':
            print('Лекарь —сильный врач. Берет силы из природы, веры и духов.')
        approve_choice = input('Y - подтвердить выбор_').lower()
    return char_class
